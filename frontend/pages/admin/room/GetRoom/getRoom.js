...
const GetRoom = () => {
  ...
  const handleConfirm = async () => {
    try {
      const getData = await axios.delete(
        `http://localhost:8000/kamar/${idRoom}`,
        config
      );
  ...
  const checkAvailable = async () => {
    try {
      const fetchData = await axios.get(
        `http://localhost:8000/kamar/checkAvailable/${tgl_akses}`,
        config
      );
      setCheck(fetchData.data.data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const openModalDetail = async (nomor_kamar) => {
    try {
      const fetchData = await axios.get(
        `http://localhost:8000/kamar/findOne/${nomor_kamar}`,
        config
      );
      setDetail(fetchData.data.data[0]);
      setOpenModal(true);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div>
      ...
      <Button
        variant="outlined"
        onClick={() => router.push("/admin/room/AddRoom")}
        sx={{
          marginTop: "2rem",
          color: "#024f79",
          borderColor: "#024f79",
                ...
                {data &&
                  currentData.map((item, index) => (
                    <StyledTableRow key={index}>
                      ...
                      <StyledTableCell align="center">
                        {check.includes(item.nomor_kamar)
                          ? "is  Available Now"
                          : "is Not Available Now"}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        <Button
                          onClick={() => openModalDetail(item.nomor_kamar)}
                        >
                          Lihat Detail
                        </Button>
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        ...
                        >
                          <Link href={`/admin/room/UpdateRoom/${item.id}`}>
                            edit
                          </Link>
                        </Button>
                        <Button
...
export default withAuth(GetRoom);
